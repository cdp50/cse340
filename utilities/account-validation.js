  ...
  return [
    ...
    body("client_email")
      ...
      .custom(async (client_email) => {
        ...
        if (emailExists) {
          throw new Error(
            "Email already exists. Please login or use different email"
          );
        }
      }),
  ...
  return [
    ...
    body("client_email")
      ...
      .custom(async (client_email) => {
        ...
        if (!emailExists) {
          throw new Error(
            "Email not registered. Please register using the link below"
          );
        }
      }),
...
validate.checkLoginData = async (req, res, next) => {
  console.log("check login data was called");
  const { client_email } = req.body;
  let errors = [];
  errors = validationResult(req);
  if (!errors.isEmpty()) {
    console.log("there are errors in login validation")
    let nav = await utilities.getNav();
